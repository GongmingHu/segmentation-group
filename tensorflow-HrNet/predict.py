...
def cut_inference_combin_color(ori_image_path,
    ...
    ori_image = cv2.imread(ori_image_path, 1)
...
if __name__ == '__main__':
    ...
    checkpoint_file = tf.train.latest_checkpoint('ckpts')
    graph = tf.Graph()
    with graph.as_default():
        ...
        param = {
            # 只有第一张图的测试结果还行
            'ori_image_path': 'dataset/top_mosaic_09cm_area1.tif', # the image you want to predict
