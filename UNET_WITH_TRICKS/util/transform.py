...
class RandomElastic(object):
    ...
    def __init__(self, p=0.5):
        self.p = p
        self.aug = ElasticTransform(alpha=1, sigma=50, alpha_affine=50, interpolation=cv2.INTER_LINEAR,
                                    border_mode=cv2.BORDER_REFLECT_101, always_apply=False, approximate=False, p=1)
...
class RandomBilateralFilter(object):
    def __init__(self, radius=7, sigmaColor=100, sigmaSpace=7, p=0.5):
        self.radius = radius
    ...
    def __call__(self, image, label):
        if random.random() < self.p:
            image = cv2.bilateralFilter(image, self.radius, self.sigmaColor, self.sigmaSpace)
        return image, label
