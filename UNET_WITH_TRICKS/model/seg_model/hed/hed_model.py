...
from model.seg_model.hed.hed_parts import make_bilinear_weights
...
class HED(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        logits = [crop1, crop2, crop3, crop4, crop5, fuse]
        out = torch.sigmoid(fuse)
        # plt.imshow(out.detach().cpu().numpy()[0, 0, :, :])
        # plt.show()

        if self.training:
            loss = sum([self.criterion(logit, y) for logit in logits])

            return fuse, out, loss
        else:
            return out
